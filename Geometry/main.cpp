...
#define SHAPE_GIVE_PARAMETERS start_x, start_y, line_width, color
	...
	{
	protected:
		unsigned int start_x, start_y, line_width;
		Color color;
	public:
	...
	public:
		...
		double get_perimeter()const override { return (width * 2) + (height * 2); }
		...
		void draw() const override
		{
			...
			//SetConsoleTextAttribute(hConsole, Color::CONSOLE_DEFAULT);*/
			HWND hwnd = GetConsoleWindow(); // получаем дескриптор окна консоил
			//HWND hwnd = FindWindow(NULL, L"Microsoft Visual Studio Debug Console"); // получаем дескриптор окна консоил
			HDC hdc = GetDC(hwnd);
			HPEN hPen = CreatePen(PS_SOLID, line_width, color+100);
	...
	class Square : public Geometry::Rectangle
	{
	public:
	...
	class Circle : public Shape
	{
		double radius;
	public:
		Circle(double radius, SHAPE_TAKE_PARAMETERS) :Shape(SHAPE_GIVE_PARAMETERS) { set_radius(radius); }
		~Circle() {}
...
void main()
{
	...
	Geometry::Square square(100, 300, 0, 8, Geometry::Color::BLUE);
	square.info();
